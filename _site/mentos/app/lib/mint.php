...
HERE;
	...
	function _checkForUpdates() 
	{
		...
		{
			$response = $this->_gateway('Request Update Ping', 'is_v130=1');
			
			// Successful connection
			if ($response) 
			{
				list($version, $url) = split(' ', $response);
				list($mint2_version, $mint1_version) = split('/', $version);
				
				$this->cfg['update']['url']			= $url;
				$this->cfg['update']['version']		= ($this->cfg['preferences']['ignoreMint2']) ? $mint1_version : $mint2_version;
	...
	function update() 
	{
		...
		if ($this->cfg['version'] < 130)
		{
			$this->cfg['preferences']['ignoreMint2'] = false;
		}
...
HERE;
	...
	function savePreferences() 
	{
		...
		$previous_ignore_setting = $this->cfg['preferences']['ignoreMint2'];
		$this->cfg['preferences']['ignoreMint2']	= (isset($_POST['ignore_mint_2']))?true:false;
		if ($previous_ignore_setting != $this->cfg['preferences']['ignoreMint2'])
		{
			$this->cfg['update']['lastPing'] = 0;
